...
public class hbookclass
{
    ...
    public string GetDHist()
    {
        ...
        sb.Append("Average : " + (sumx / total).ToString() + "\n");
        sb.Append("Sigma : " + getsigma(total).ToString() + "\n");
        return sb.ToString();
    }

    public double getsigma(int total)
    {
        double sig = Math.Sqrt(total * sumx2 - sumx * sumx) / total;
        return sig;
    }
